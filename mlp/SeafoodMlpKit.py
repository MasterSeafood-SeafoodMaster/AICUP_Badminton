...
def DataLoader_RoundHead(txtroot, csvroot):
    ...
    for folders in rootFolders:
        ...
        for i, f in enumerate(HitFrame):
            exHitFrame = expand_array(np.array([f]), 0, low)
            for ef in exHitFrame:
              if WhosHit[i]=="A":
                  X.append(p0_arr[ef].tolist()+ball[ef].tolist())
              elif WhosHit[i]=="B":
                  X.append(p1_arr[ef].tolist()+ball[ef].tolist())
...
def DataLoader_BackHead(txtroot, csvroot):
    X=[]; y=[]
    rootFolders = os.listdir(txtroot)
    progress = tqdm(total=len(rootFolders))
    for folders in rootFolders:
        ...
        HitFrame = classes[1:, 1].astype(int)
        WhosHit = classes[1:, 2].astype(str)
        BackHead = classes[1:, 4].astype(str)
...
def DataLoader_BallHeight(txtroot, csvroot):
    X=[]; y=[]
    rootFolders = os.listdir(txtroot)
    progress = tqdm(total=len(rootFolders))
    for folders in rootFolders:
        ...
        HitFrame = classes[1:, 1].astype(int)
        WhosHit = classes[1:, 2].astype(str)
        BackHead = classes[1:, 5].astype(str)
...
def DataLoader_Landing(txtroot, csvroot):
    X=[]; y=[]
    rootFolders = os.listdir(txtroot)
    progress = tqdm(total=len(rootFolders))
    for folders in rootFolders:
        ...
        HitFrame = classes[1:, 1].astype(int)
        WhosHit = classes[1:, 2].astype(str)
        Landing = norm(classes[1:, 6:8].astype(float))
        #print(Landing)
        

        for i, f in enumerate(HitFrame):
            if i+1<len(HitFrame):
                X.append(court[f].tolist()+ball[f+1].tolist())
            else:
                X.append(court[f].tolist()+ball[len(ball)-1].tolist())
            
            y.append(Landing[i].tolist())
...
def DataLoader_Loc(txtroot, csvroot):
    X=[]; y=[]
    rootFolders = os.listdir(txtroot)
    progress = tqdm(total=len(rootFolders))
    for folders in rootFolders:
        ...
        HitFrame = classes[1:, 1].astype(int)
        WhosHit = classes[1:, 2].astype(str)
        HitterLoc = norm(classes[1:, 8:10].astype(float))
        DefenderLoc = norm(classes[1:, 10:12].astype(float))
        #print(Landing)
        

        for i, f in enumerate(HitFrame):
            if WhosHit[i]=="A":
                X.append(court[f].tolist()+p0_arr[f].tolist())
                y.append(HitterLoc[i].tolist())
                X.append(court[f].tolist()+p1_arr[f].tolist())
                y.append(DefenderLoc[i].tolist())
...
def DataLoader_firstBalltype(txtroot, csvroot):
    #txtroot = "../Dataset_badminton/transfer/"
    #csvroot = "../Dataset_badminton/train/"
    ...
    for folders in rootFolders:
        ...
        HitFrame = classes[1:, 1].astype(int)
        WhosHit = classes[1:, 2].astype(str)
        Balltype = classes[1, 12].astype(int)

        #print(Balltype)
        #for i, f in enumerate(HitFrame):
        i=0
        f = HitFrame[i]
        ball10 = get_ball_coordinates_f10(ball, f).reshape(22, )
        if WhosHit[i]=="A":
            X.append(p0_arr[f].tolist()+ball10.tolist())
            y.append(Balltype-1)
        elif WhosHit[i]=="B":
...
def DataLoader_Balltype(txtroot, csvroot):
    #txtroot = "../Dataset_badminton/transfer/"
    #csvroot = "../Dataset_badminton/train/"
    ...
    for folders in rootFolders:
        ...
        low = min(court_arr.shape[0], p0_arr.shape[0], p1_arr.shape[0])
        HitFrame = classes[2:, 1].astype(int)
        WhosHit = classes[2:, 2].astype(str)
        Balltype = classes[2:, 12].astype(int)-3
...
def norm(arr):
    result = arr.copy()  # 創建輸入陣列的副本，以避免修改原始陣列
